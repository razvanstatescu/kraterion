...
export class BillingService {
  ...
  /**
   * Build a **`mode: 'setup'`** Checkout Session for a project — the
   * Vercel / Supabase / AWS shape. Stripe collects the card and
   * attaches it to the Customer. The subscription is created
   * **server-side** in the webhook handler once we know the card is
   * good (`checkout.session.completed`).
   *
   * What the user sees: one Stripe-hosted card form. No plan picker,
   * no "subscribe" button, no line-item preview. They click save and
   * land back on the dashboard. Billing happens monthly behind the
   * scenes based on meter events.
   *
   * Why server-side subscription creation:
   *   - Lets us pick the line-item composition without surfacing it
   *     in the Checkout UI.
   *   - Keeps the catalog versioned in code, not in the URL.
   *   - The webhook is the natural source-of-truth boundary: if Stripe
   *     says "card attached", we create the subscription; if it
   *     doesn't, we don't.
   */
  async createCheckoutSession(args: {
    ...
    const session = await this.stripe.client.checkout.sessions.create(
      {
        ...
        client_reference_id: args.projectId,
        metadata: {
          project_id: args.projectId,
          kraterion_mode: this.stripe.mode,
        },
        // The card collected here becomes the customer's default for
        // off-session subscription invoices.
        setup_intent_data: {
          metadata: {
            project_id: args.projectId,
            kraterion_mode: this.stripe.mode,
      ...
      {
        idempotencyKey: `checkout-setup:${args.projectId}:${Date.now() >> 20}`,
        // ^ bucketed by ~17 min so honest retries collapse, but the
        // user can re-open Checkout after that without hitting a
        // cached session URL.
      },
    );
    if (!session.url) {
    ...
    this.logger.log(
      `created setup Checkout session ${session.id} for project=${args.projectId}`,
    );
