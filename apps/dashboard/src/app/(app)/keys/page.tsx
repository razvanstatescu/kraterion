    ...
    <>
      ...
      <main className="ks-screen">
        <div className="ks-screen-head">
          <div>
            <h1>Access keys</h1>
            <p className="lead" style={{ fontSize: 14, marginTop: 4, maxWidth: 600 }}>
              Use access keys with any S3-compatible client &mdash; boto3, aws-cli,
              rclone. We never see the secret after you create it.
            </p>
          </div>
        </div>
        ...
        {error ? (
          ...
          <EmptyState
            ...
            action={<Button variant="cta" icon="plus" onClick={() => setCreateOpen(true)}>New key</Button>}
          />
        ) : (
          <>
                  ...
                  return (
                    ...
                    >
                      <div style={{ flex: "2 1 0", display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                        <Icon name="key" size={16} style={{ color: "var(--text-secondary)", flexShrink: 0 }} />
            ...
            {/* Persistent quickstart for the most recent active key.
                The secret is gone (only shown at mint time) — the
                snippet uses a `<your-secret-shown-once-at-creation>`
                placeholder. Mint a new key to see it again. */}
            {active[0] ? (
              <>
